...
class Queen: Piece {
    ...
    override init(color: Color?, position: Square?, hasMoved: Bool?, delegate: PieceDelegate?) {
        ...
        symbol = kQueenSymbol
        value = abs(kQueenValue) * color!.rawValue
    ...
    override func move(_ toSquare: Square) -> Bool {
        return (Rook.move(Move(fromSquare: position!, toSquare: toSquare)) || Bishop.move(Move(fromSquare: position!, toSquare: toSquare))) && Board.sharedInstance.checkForClearPath(Move(fromSquare: position!, toSquare: toSquare))
    }
}
